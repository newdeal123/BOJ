...
void f(int n, int p, int s, int m, int d, int sum)
{
	if (n == N)
	{
		Max = Max > sum ? Max : sum;
	...
	if (p)
		 f(n + 1, p - 1, s, m, d, sum + arr[n + 1]);
	if (s)
		 f(n + 1, p, s - 1, m, d, sum - arr[n + 1]);
	if (m)
		f(n + 1, p, s, m - 1, d, sum*arr[n + 1]);
	if (d)
		f(n + 1, p, s, m, d - 1, sum / arr[n + 1]);
}
