...
int happiness(int idx, int heart)
{
	if (idx > N)
		return 0;

	int ret = 0;
	if (heart - decrease[idx] > 0)
		return ret = max(happiness(idx + 1, heart), happiness(idx + 1, heart - decrease[idx]) + increase[idx]);
	else
		return ret = happiness(idx + 1, heart);
}
