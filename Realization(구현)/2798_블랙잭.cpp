...
int f(int idx, int sum, int selected)
{
	if (selected == 3)
		return sum;
	if (idx > N)
	...
	if (selected < 3 && sum + arr[idx] <= M)
		return ret = max(f(idx + 1, sum + arr[idx], selected + 1), f(idx + 1, sum, selected));
	else
		return ret = max(ret, f(idx + 1, sum, selected));
}
